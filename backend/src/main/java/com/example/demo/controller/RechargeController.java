...
public class RechargeController {
    ...
    public ResponseEntity<?> recharge(@RequestBody RechargeRequest request) {
        try {
            ...
            // Safe handling of plan.getDataPerDay()
            String dataPerDayStr = plan.getDataPerDay();
            double dataTotal = 0.0;
            if (dataPerDayStr != null && dataPerDayStr.contains("GB/day")) {
                try {
                    double dataPerDay = Double.parseDouble(dataPerDayStr.replace("GB/day", "").trim());
                    dataTotal = dataPerDay * plan.getValidityDays();
                } catch (NumberFormatException e) {
                    return ResponseEntity.badRequest().body("Invalid data per day format in plan");
                }
            } else {
                return ResponseEntity.badRequest().body("Plan dataPerDay is missing or invalid");
            }
            subscriber.setDataTotal(dataTotal);
            ...
            try {
                ...
                e.printStackTrace();
                return ResponseEntity.status(500).body("Recharge succeeded but failed to send confirmation email: " + e.getMessage());
            }
